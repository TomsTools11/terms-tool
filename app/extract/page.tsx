...
import { saveTerms, checkDuplicates } from '@/lib/storage';
  ...
  const extractTerms = async (transcript: string) => {
    ...
    try {
      ...
      extractedTerms.forEach(term => {
        const existing = checkDuplicates(term.term);
        ...
        <div className="flex items-start gap-3 p-4 bg-[var(--color-warning)]/10 border border-[var(--color-warning)]/30 rounded-lg">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-text-muted)] mt-0.5">
              Some terms already exist in your glossary. Saving will update existing definitions.
            </p>
          </div>
        </div>
      )}
